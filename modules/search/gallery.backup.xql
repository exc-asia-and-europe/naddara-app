...
import module namespace mods-hra="http://hra.uni-heidelberg.de/ns/mods-hra-framework" at "/db/apps/tamboti/frameworks/mods-hra/mods-hra.xqm";
...
declare function local:_get-collection-resources() as xs:string* {
    let $collection := request:get-parameter("collection",()) return
        xmldb:get-child-resources($collection)[fn:lower-case(fn:replace(. , ".*\.", "")) = $local:image-exts]
};
...
declare function local:thumbnails($filmstrip-width as xs:int, $page as xs:int, $fn-images-available, $fn-get-images, $fn-get-image-collection) {
    ...
    if ($item) then
        ...
        let $fn :=
            if(request:get-parameter("collection", ())) then
                (
                    local:get-collection-image-count#0,
                    local:get-images-from-collection#2,
                    local:get-image-collection-for-collection#1
                )
            else
                (
